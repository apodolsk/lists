/**
 * A lockfree doubly-linked list. It only supports addition at its tail,
 * but is perfectly robust.
 *
 * As in a single threaded dllist, each flanchor stores next and prev
 * pointers. Every function likewise aims to perform the same steps as in
 * the single threaded version. Helping and shootdown 
 *
 * The hard part is deletion. del(A) must set things up so it can use a
 * CAS2 to write P->n.pt = N only if A->p.pt == P && A->n.pt == N &&
 * P->n.pt == A && N->p.pt == A. Let D(A) be the set of threads committed
 * to writing 'P->n.pt = N' only if P->n.pt == A, for any P' and N'.
 * 
 * The scheme is that once a del(a) has fetched some likely-looking P and
 * N and has decided to attempt this write, it sets A->n.st = COMMIT and
 * updates A->n.gen. At this point, A is called locked. If A is locked,
 * all other lflist functions will attempt to help del(A) before writing
 * A->n.pt.
 *
 * A note: this is so far like the lockfree list by Hakan and Tsigas,
 * where A would have a lock bit called A->n.deleting. If the lock bit is
 * set, A->n may not be written. In order for del(A->n) to make progress
 * in light of this, it must follow the chain of prev pointers "back" from
 * A until it finds a node PP s.t. !PP->n.deleting. I hope it's
 * illustrative to imagine how a list might be feasible in this simpler
 * scheme where del(A) could guarantee that A->n is immutable.
 *
 * The key idea of my list is that it provides a similar but looser
 * invariant: if a thread locks A and succeeds writing writing [P->n.pt =
 * N iff A->p.pt == P && P->n.pt == A], (and A is not subsequently
 * re-enqueued) then A->n.pt must be unchanged. In other words, a
 * help_prev may unlock A->n, but only if every thread committed to a
 * write attempt on behalf of A is bound to fail.
 *
 * A->n.gen facilites this. If a thread wishes to write A->n.pt but reads
 * it in help_prev and finds it locked, it reads apn = A->p.pt->n. If apn
 * == A and is unlocked, then it attempts D(A) 
 *
 *
 * 
 * A->n.st == COMMIT, A->n.pt may be modified iff every del(A) which has
 * not yet (re)read A->n will fail every
 *
 * But in this list, 
 * 
 *
 * In fact,
 * the key is that it won't be written to unless all del(a) 
 * 
 * it won't be written to unless the 
 * 
 * The list maintains the invariant that, if A->n.st ==
 * COMMIT, it won't be written to if the del set A->n.st can 
 * 
